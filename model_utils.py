...
#sxysxy
            for name in target_layer_names:
                ...
                print('Pruning ...')
                sparsity = args.sparsity
                gpts[name].prob_structured_iterative_correction_prune(
                    sparsity, prunen=args.prunen, prunem=args.prunem, percdamp=args.percdamp, blocksize=args.blocksize
                )
    ...
    for i in range(len(layers)):
        ...
        for names in sequential:
            ...
            for j in range(args.nsamples):
                ...
                # Build position_ids on the same device
                seq_len = hidden_states.shape[1]
                position_ids = torch.arange(seq_len, device=dev).unsqueeze(0)

                # This returns a 2-tuple (cos, sin)
                cos_sin_tuple = model.model.rotary_emb(hidden_states, position_ids)

                # Pass that tuple as position_embeddings
                outs[j] = layer(
                    hidden_states,
...
#sxysxy
        ...
        for j in range(args.nsamples):
            ...
            seq_len = hidden_states.shape[1]
            position_ids = torch.arange(0, seq_len, device=dev).unsqueeze(0)

            # Manually create (cos, sin)
            cos_sin_tuple = model.model.rotary_emb(hidden_states, position_ids)
            # cos_sin_tuple is (cos, sin)

            outs[j] = layer(
                hidden_states,
                attention_mask=attention_mask,
                position_embeddings=cos_sin_tuple,  # <--- pass the tuple
            )[0]            
...
def llama_eval(model, testenc, dev, args, dataset: str):
    ...
    for i in range(len(layers)):
        ...
        for j in range(nsamples):
            # (seq_len, hidden_size)
            hidden_for_rope = inps[j].unsqueeze(0).to(dev)
            chunk_seq_len   = hidden_for_rope.shape[1]

            # Build position_ids on GPU
            position_ids = torch.arange(0, chunk_seq_len, dtype=torch.long, device=dev)
            position_ids = position_ids.unsqueeze(0)  # shape (1, seq_len)

            # Rotary embeddings also on GPU
